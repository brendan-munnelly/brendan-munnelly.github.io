/* Text Content Assets */

/* Label in single-column layout */
export const content_header_label_text_col_1 ="PRODUCTS";

/* H2 text in single-column */
export const content_header_h2_text_col_1 ="Nice section heading";

/* H2 element in single-column */
export const content_header_h2_col_1 ="<h2>"+content_header_h2_text_col_1+"</h2>";

/* H3 as small sub-heading in single-column layout */
export const content_header_h3_col_1 ="<h3 class=\"in-col-1\">Nice sub-heading here<\/h3>\n\t\t";

/* H3 as sub-headings in 2, 3 and 4 column layouts */
export const content_header_h3_text_col_2_3_4 ="Leverage agile frameworks to provide a robust synopsis for high level overviews to foster collaborative thinking.";

/* Entire upper block in 2, 3 and 4 column layouts */
export const content_header_upper_block_col_2_3_4 ="\n\t\t\t<h2>"+content_header_h2_text_col_1+"</h2>\n\t\t\t<h3>"+content_header_h3_text_col_2_3_4+"</h3>\n\t\t";

/* Single column layouts */
export const content_para_last_col_1 = "\n\n\t\t<p>Collaboratively administrate empowered markets via plug-and-play networks. Dynamically procrastinate B2C users after installed base benefits. Bring to the table win-win survival strategies to ensure proactive domination.<\/p>";

export const content_ul_short_col_1 = "\n\n\t\t<ul>\n\t\t\t<li>AI-powered<\/li>\n\t\t\t<li>Future-proof<\/li>\n\t\t\t<li>Customer-centric<\/li>\n\t\t<\/ul>";

export const content_ul_long_col_1 = "\n\n\t\t<ul>\n\t\t\t<li><b>Fast delivery<\/b>: We ship reliably all over Europe using only the best and fastest delivery services.<\/li>\n\t\t\t<li><b>Lowest prices<\/b>: We shine on product quality and we will not be beaten on product prices.<\/li>\n\t\t\t<li><b>Our mission<\/b>: We help our clients reinvent their brands, products, and culture for a changing world.<\/li>\n\t\t<\/ul>";

export const content_caption_col_1 = "<figcaption><i>Above<\/i>: Some text describing the image or video.</figcaption>";

/* 2 column layouts */
export const content_h3_col_2 = ["\n\t\t\t<h3>Lunevedy Cafe Bakery<\/h3>", "\n\t\t\t<h3>Lunevedy Fashion Store<\/h3>"];

export const content_lists_col_2 = ["\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship reliably to almost everywhere using only the best and fastest delivery services.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We shine on product quality and we will not be beaten on product prices.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help our clients reinvent their brands, products, and culture for a changing world.<\/li>\n\t\t\t<\/ul>", "\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship reliably all over Europe using only the best and fastest delivery services.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We shine on product quality and we will not be beaten on product prices.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help our clients reinvent their brands, products, and culture for a changing world.<\/li>\n\t\t\t<\/ul>"];

export const content_paras_col_2 = ["\n\t\t\t<p>Lunevedy&#8219;s is a lively landmark city cafe bar, with great food and drink served in a unique atmosphere.</p>\n\t\t\t<p>Our friendly baristas are waiting to pour your favourite beverage. We use only hand roasted coffee and expertly selected tea leaves.<\/p>", "\n\t\t\t<p>Shop our collection of luxury women&#8219;s clothing, exquisite labels, real fit, individuality and personal service.</p>\n\t\t\t<p>Level up your look with our ever-changing selection of on-trend brands. We ship to almost everywhere quickly and reliably.</p>"];

/* 2 column split layouts */
export const content_para_col_2 ="<p>Further the overall value proposition. Organically grow the holistic world view of disruptive innovation. Identify a ballpark value added activity to beta test.</p>";

export const content_list_col_2 = ["\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship reliably to almost everywhere using only the best and fastest delivery services.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We shine on product quality and we will not be beaten on product prices.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help our clients reinvent their brands, products, and culture for a changing world.<\/li>\n\t\t\t<\/ul>"];


/* buttons for single-column, 2 column, and 2-column split layouts */
export const content_buttons_one_icon = "<i class=\"fas fa-arrow-right\"></i>";
export const content_buttons_one_text = "Start free trial";

export const content_buttons_pair_icon_1 = "<i class=\"fas fa-shopping-cart\"></i>";
export const content_buttons_pair_text_1 = "Shop now";

export const content_buttons_pair_icon_2 = "<i class=\"fas fa-arrow-right\"></i>";
export const content_buttons_pair_text_2 = "View details";


export const content_buttons_pair = "\n\n\t<div class=\"container-btn\">\n\t\t<a href=\"#\" class=\"btn btn-solid\"><i class=\"fas fa-shopping-cart\"></i><span>Order now<\/span></a>\n\t\t<a href=\"#\" class=\"btn btn-outline\"><i class=\"fas fa-arrow-right\"></i><span>Learn more<\/span></a>\n\t</div>";

/* 3 column layouts */
export const content_h3_col_3 = ["\n\t\t\t<h3>Lunevedy Cafe Bakery<\/h3>", "\n\t\t\t<h3>Lunevedy Fashion Store<\/h3>", "\n\t\t\t<h3>Learn JavaScript today<\/h3>"];

export const content_paras_col_3 = ["\n\t\t\t<p>Our friendly baristas are waiting to pour your favourite beverage. We use only hand roasted coffee and expertly selected tea leaves.<\/p>", "\n\t\t\t<p>Level up your look with our ever-changing selection of on-trend brands. We ship to almost everywhere quickly and reliably.<\/p>", "\n\t\t\t<p>Supercharge your web design career with this <i>JavaScript for Beginners<\/i> training course that covers all the important concepts.<\/p>"];

export const content_list_col_3 = ["\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship to almost everywhere quickly and reliably.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We shine on product quality and we will not be beaten on product prices.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help our clients reinvent their brands, products, and culture for a changing world.<\/li>\n\t\t\t<\/ul>", "\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship to almost everywhere quickly and reliably.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We shine on product quality and we will not be beaten on product prices.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help our clients reinvent their brands, products, and culture for a changing world.<\/li>\n\t\t\t<\/ul>", "\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship to almost everywhere quickly and reliably.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We shine on product quality and we will not be beaten on product prices.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help our clients reinvent their brands, products, and culture for a changing world.<\/li>\n\t\t\t<\/ul>"];

/* 4 column layouts */
export const content_h3_col_4 = ["\n\t\t\t<h3>Lunevedy Cafe<\/h3>", "\n\t\t\t<h3>Lunevedy Fashion<\/h3>", "\n\t\t\t<h3>Learn JavaScript<\/h3>", "\n\t\t\t<h3>Visit Portugal<\/h3>"];

export const content_paras_col_4 = ["\n\t\t\t<p>Our friendly baristas are waiting to serve you. We use only hand roasted coffee and expertly selected tea leaves.<\/p>", "\n\t\t\t<p>Level up your look with our ever-changing selection of on-trend brands. We ship to almost everywhere.<\/p>", "\n\t\t\t<p>Supercharge your web design career with this <i>JavaScript for Beginners</i> course that covers all the important concepts.<\/p>", "\n\t\t\t<p>Portugal's hilly, coastal capital city. Enjoy the pastel-coloured buildings, the cobbled streets, and the custard tarts.<\/p>"];

export const content_list_col_4 = ["\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship to almost everywhere.<\/li>\n\t\t\t\t<li><b>Best value<\/b>: We will not be beaten on price.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help client reinvent their brands.<\/li>\n\t\t\t<\/ul>", "\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship to almost everywhere.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We will not be beaten on price.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help client reinvent their brands.<\/li>\n\t\t\t<\/ul>", "\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship to almost everywhere.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We will not be beaten on price.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help client reinvent their brands.<\/li>\n\t\t\t<\/ul>", "\n\t\t\t<ul>\n\t\t\t\t<li><b>Fast delivery<\/b>: We ship to almost everywhere.<\/li>\n\t\t\t\t<li><b>Lowest prices<\/b>: We will not be beaten on price.<\/li>\n\t\t\t\t<li><b>Our mission<\/b>: We help client reinvent their brands.<\/li>\n\t\t\t<\/ul>"];




export const arrPic = [];
arrPic[0]="\n\t\t\t<figure>\n\t\t\t\t<img src=\"assets\/img\/460x280-bakery.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";
arrPic[1]="\n\t\t\t<figure>\n\t\t\t\t<img src=\"assets\/img\/460x280-fashion-yellow.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";
arrPic[2]="\n\t\t\t<figure>\n\t\t\t\t<img src=\"assets\/img\/460x280-student.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";
arrPic[3]="\n\t\t\t<figure>\n\t\t\t\t<img src=\"assets\/img\/460x280-lisbon.jpg\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";

export const arrTrans = [];
arrTrans[0]="\n\t\t\t<figure>\n\t\t\t\t<img src=\"assets\/img\/460x300-trans-couple-coffee.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";
arrTrans[1]="\n\t\t\t<figure>\n\t\t\t\t<img src=\"assets\/img\/460x300-trans-woman-smiling.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";
arrTrans[2]="\n\t\t\t<figure>\n\t\t\t\t<img src=\"assets\/img\/460x300-trans-man-laptop.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";
arrTrans[3]="\n\t\t\t<figure>\n\t\t\t\t<img src=\"assets\/img\/460x300-trans-lisbon.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";

export const arrIllus = [];  
arrIllus[0]="\n\t\t\t<figure>\n\t\t\t\t\t<img src=\"assets\/img\/500x340-drawing-couple-dining.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";
arrIllus[1]="\n\t\t\t<figure>\n\t\t\t\t\t<img src=\"assets\/img\/500x340-drawing-shopping.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";
arrIllus[2]="\n\t\t\t<figure>\n\t\t\t\t\t<img src=\"assets\/img\/500x340-drawing-education.png\" alt=\"Placeholder image\"><\/figure>";
arrIllus[3]="\n\t\t\t<figure>\n\t\t\t\t\t<img src=\"assets\/img\/500x340-drawing-lisbon.png\" alt=\"Placeholder image\">\n\t\t\t<\/figure>";

export const arrVidFile = [];  
arrVidFile[0]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-file\"><video controls=\"\"><source src=\"assets/videos/1280x720-coffee-cafe.mp4\" type=\"video\/mp4\"><\/video><\/div>\n\t\t\t<\/figure>";
arrVidFile[1]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-file\"><video controls=\"\"><source src=\"assets/videos/1280x720-fashion.mp4\" type=\"video\/mp4\"><\/video><\/div>\n\t\t\t<\/figure>";
arrVidFile[2]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-file\"><video controls=\"\"><source src=\"assets/videos/1280x720-software.mp4\" type=\"video\/mp4\"><\/video><\/div>\n\t\t\t<\/figure>";
arrVidFile[3]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-file\"><video controls=\"\"><source src=\"assets/videos/1280x720-lisbon.mp4\" type=\"video\/mp4\"><\/video><\/div>\n\t\t\t<\/figure>";

export const arrVidYT = [];  
arrVidYT[0]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-yt\"><iframe src=\"https://www.youtube.com/embed/qPPlBpRkeAo\" title=\"YouTube video player\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"\" width=\"560\" height=\"315\" frameborder=\"0\"><\/iframe><\/div>\n\t\t\t<\/figure>";
arrVidYT[1]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-yt\"><iframe src=\"https://www.youtube.com/embed/MVNxViRQrF8\" title=\"YouTube video player\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"\" width=\"560\" height=\"315\" frameborder=\"0\"><\/iframe><\/div>\n\t\t\t<\/figure>";
arrVidYT[2]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-yt\"><iframe src=\"https://www.youtube.com/embed/u044iM9xsWU\" title=\"YouTube video player\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"\" width=\"560\" height=\"315\" frameborder=\"0\"><\/iframe><\/div>\n\t\t\t<\/figure>";
arrVidYT[3]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-yt\"><iframe src=\"https://www.youtube.com/embed/N8bHCHl8X_0\" title=\"YouTube video player\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"\" width=\"560\" height=\"315\" frameborder=\"0\"><\/iframe><\/div>\n\t\t\t<\/figure>";

export const arrVidRumble = [];  
arrVidRumble[0]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-yt\"><iframe src=\"https://rumble.com/embed/v168c2l/?pub=4\" title=\"Rumble video player\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"\" width=\"560\" height=\"315\" frameborder=\"0\"><\/iframe><\/div>\n\t\t\t<\/figure>";
arrVidRumble[1]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-yt\"><iframe src=\"https://rumble.com/embed/v168c2l/?pub=4\" title=\"Rumble video player\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"\" width=\"560\" height=\"315\" frameborder=\"0\"><\/iframe><\/div>\n\t\t\t<\/figure>";
arrVidRumble[2]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-yt\"><iframe src=\"https://odysee.com/$/embed/did-google%E2%80%99s-a.i.-just-become-sentient/14d83b5976ae7f5a4b94e91ec1391bafc435e2ae?r=95Jvfn3aYo3zUzf6omjQZsMoi7ESTeyZ\" title=\"Rumble video player\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"\" width=\"560\" height=\"315\" frameborder=\"0\"><\/iframe><\/div>\n\t\t\t<\/figure>";
arrVidRumble[3]="\n\t\t\t<figure>\n\t\t\t\t\t<div class=\"container-video-yt\"><iframe src=\"https://rumble.com/embed/v168c2l/?pub=4\" title=\"Rumble video player\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen=\"\" width=\"560\" height=\"315\" frameborder=\"0\"><\/iframe><\/div>\n\t\t\t<\/figure>";

export const arrIconFA = [];             
arrIconFA[0]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"fa-solid fa-user-group\"></i>\n\t\t\t\<\/figure>";
arrIconFA[1]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"fa-regular fa-comments\"></i>\n\t\t\t\<\/figure>";
arrIconFA[2]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"fa-regular fa-lightbulb\">\n\t\t\t</i><\/figure>";
arrIconFA[3]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"fa-solid fa-plane-departure\"></i>\n\t\t\t\<\/figure>";

export const arrIconLA = [];             
arrIconLA[0]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"las la-user-friends\"><\/i>\n\t\t\t<\/figure>";
arrIconLA[1]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"las la-comments\"><\/i>\n\t\t\t<\/figure>";
arrIconLA[2]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"las la-lightbulb\"><\/i>\n\t\t\t<\/figure>";
arrIconLA[3]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"las la-plane-departure\"><\/i>\n\t\t\t<\/figure>";
arrIconLA[4]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"las la-globe\"><\/i>\n\t\t\t<\/figure>";
arrIconLA[5]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"las la-shopping-cart\"><\/i>\n\t\t\t<\/figure>";   
arrIconLA[6]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"las la-briefcase\"><\/i>\n\t\t\t<\/figure>";
arrIconLA[7]="\n\t\t\t<figure class=\"icon\">\n\t\t\t\t<i class=\"las la-book-reader\"><\/i>\n\t\t\t<\/figure>"; 
